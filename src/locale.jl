...
struct Locale
    ...
    locale_extensions::LocaleExtensions
end

const LocaleKey = Tuple{NTuple{4,Symbol},Vararg{Any}}
const LOCALECACHE = Dict{LocaleKey,Locale}

    # Useful constant for language.
    const ENGLISH = createConstant("en", "");

    # Useful constant for language.
    const FRENCH = createConstant("fr", "");

    # Useful constant for language.
    const GERMAN = createConstant("de", "");

    # Useful constant for language.
    const ITALIAN = createConstant("it", "");

    # Useful constant for language.
    const JAPANESE = createConstant("ja", "");

    # Useful constant for language.
    const KOREAN = createConstant("ko", "");

    # Useful constant for language.
    const CHINESE = createConstant("zh", "");

    # Useful constant for language.
    const SIMPLIFIED_CHINESE = createConstant("zh", "CN");

    # Useful constant for language.
    const TRADITIONAL_CHINESE = createConstant("zh", "TW");

    # Useful constant for country.
    const FRANCE = createConstant("fr", "FR");

    # Useful constant for country.
    const GERMANY = createConstant("de", "DE");

    # Useful constant for country.
    const ITALY = createConstant("it", "IT");

    # Useful constant for country.
    const JAPAN = createConstant("ja", "JP");

    # Useful constant for country.
    const KOREA = createConstant("ko", "KR");
    ...
    # Useful constant for country.
    const UK = createConstant("en", "GB");

    # Useful constant for country.
    const US = createConstant("en", "US");

    # Useful constant for country.
    const CANADA = createConstant("en", "CA");
...
const ROOT = createConstant("", "");
     ...
     * @since 1.7
"""     */
const UNICODE_LOCALE_EXTENSION = 'u'
    # Display types for retrieving localized names from the name providers.
    const DISPLAY_LANGUAGE = 0
    const DISPLAY_COUNTRY  = 1
    const DISPLAY_VARIANT  = 2
    const DISPLAY_SCRIPT   = 3

"""
     * Construct a locale from language, country and variant.
     * This constructor normalizes the language value to lowercase and
     * the country value to uppercase, the script to titlecase.
     * The input fields are checked for syntactic correctness.
     * <p>
     * <b>Note:</b>
     * <ul>
...
"""
function Locale(lang::AbstractString, scri::AbstractString, regi::AbstractString, vari::AbstractString, ext = nothing)
        base_locale = BaseLocale(lang, scri, regi, vari)
        ext == nothing && ( ext = get_compatibility_extensions(lang, scri, regi, vari))
        get!(LOCALECACHE, key) do
            Locale(base_locale, ext)
        end
end

function Locale(language::AbstractString, country::AbstractString, variant::AbstractString)
    Locale(language, "", country, variant)
end

Locale(AbstractString language, AbstractString country) = Locale(language, country, "")
Locale(language::AbstractString) == Locale(language, "", "")

createConstant(String lang, String country) = Locale(lang, country)
